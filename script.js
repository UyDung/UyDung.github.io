...
for (let i = 0; i < questions.length; i++) {
      questions[i].addEventListener("click", () => {
            if (
                  questions[i].querySelector(".answer").style.display ===
                  "block"
            ) {
...
const checkEmail = () => {
      const pattern =
            /[a-zA-Z0-9]{5,30}@([a-zA-Z0-9]{2,30})\.[a-zA-Z0-9]{2,30}/;
...
const  humberger_content = document.querySelector('.humberger_content');
const  close = document.getElementById('close');

humberger.addEventListener('click', () => {
      humberger_content.style.display = 'flex';
})

close.addEventListener('click', () => {
      humberger_content.style.display = 'none';
})